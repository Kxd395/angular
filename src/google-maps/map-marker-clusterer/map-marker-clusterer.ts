...
/** Gets the clusterer API from the window and types it correctly. */
function getApi(): {
...
export class MapMarkerClusterer implements OnInit, OnChanges, AfterContentInit, OnDestroy {
  ...
  private _createCluster() {
    if (!getApi()?.MarkerClusterer && (typeof ngDevMode === 'undefined' || ngDevMode)) {
      throw Error(
    ...
    this._ngZone.runOutsideAngular(() => {
      this.markerClusterer = new (getApi().MarkerClusterer)({
        ...
        onClusterClick: (event, cluster, map) => {
          if (this.clusterClick.observers.length) {
            ...
            getApi().defaultOnClusterClickHandler(event, cluster, map);
          }
        },
      });
    });
